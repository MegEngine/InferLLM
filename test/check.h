...
void check_tensor(const Tensor& computed, const Tensor& expected, float epsilon = 1e-3,
...
float Checker<Opr>::exec(const TensorShapeArray& shapes) {
  ...
  auto init_input = [this, &shapes](CheckerHelper::TensorValueArray& inputs, int& index) {
  ...
  auto init_weight = [this, &shapes](CheckerHelper::TensorValueArray& weights, int& index) {
